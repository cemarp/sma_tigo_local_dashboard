...
DEFAULT_USER_GROUP = "usr"
DEFAULT_PASSWORD = "User!2345"
DEFAULT_DB_FILE = "sma_inverter_data.db"
...
def extract_telemetry(session, base_url, sid, metadata, en_us):
    ...
    for sensor_key, channels in device_telemetry.items():
        ...
        category_path_lower = [en_us.get(str(h_id), str(h_id)).lower() for h_id in tag_hier]

        target_category = ""
        # Precise category mapping
        if "dc side" in category_path_lower and "dc measurements" in category_path_lower:
            if "pv module electronics" in category_path_lower:
        ...
        elif "module technology" in category_path_lower:
            target_category = "Module Technology"
